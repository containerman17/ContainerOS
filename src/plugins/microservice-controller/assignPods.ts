...
export default async function (microserviceList: keyable<StoredMicroservice>) {
    for (let [msName, microservice] of Object.entries(microserviceList)) {
        ...
        await database.microservice.safePatch(msName, (oldMicroservice: StoredMicroservice): StoredMicroservice => {
            ...
            if (diff > 0) {
                ...
                const selectedNode = database.nodeHealth.getLeastBusyServerName()
